...
[start=11]
. We are going to now set a key so this vm will run on the same node as node-affinity-vm. Set the key to app and values to fedora. Your Final Pod Affinity rule will look like the picture below. Confirm this is true and click Save affinity rule.
