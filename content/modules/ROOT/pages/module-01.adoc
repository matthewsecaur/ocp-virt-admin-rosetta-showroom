...
[start=5]
. Change the selection to Specific Node and then check the box next to a node (e.g. `worker-cluster-s67qr-1`. Your lab will have a different node name).
...
Note that the worker node is now on the new worker node (e.g. `worker-cluster-s67qr-1`).
